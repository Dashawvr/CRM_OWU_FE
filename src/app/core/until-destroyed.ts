...
export function untilDestroyed(instance: object, destroyMethodName: string = 'ngOnDestroy'): <T>(source: Observable<T>) => Observable<T> {
  return <T>(source: Observable<T>) => {
    ...
    if (!instance[untilDestroyedSymbol]) {
      ...
      instance[destroyMethodName] = function(): void {
        if (hasDestroyFunction) {
